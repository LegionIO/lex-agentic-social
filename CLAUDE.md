...
`Social::Consent` governs whether actions require human approval. The HITL gate for autonomous promotion must not be bypassed. `Social::Governance` provides the council approval mechanism used by `Defense::Extinction` for containment escalation.
